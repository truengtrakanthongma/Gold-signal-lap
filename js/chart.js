...
export class Chart {
  ...
  _draw() {
     ...
     * ── ตัดขอบแผงราคา ───────────────────────────────────────────────────
     *
     * ทุกอย่างที่แปลงราคาเป็นตำแหน่งแนวตั้ง ต้องถูกตัดให้อยู่ในแผงราคาเท่านั้น
     *
     * ก่อนมีการยืดแกนราคา กรอบ min/max ครอบทุกแท่งที่มองเห็นอยู่แล้ว
     * จึงไม่มีอะไรล้นออกไป พอผู้ใช้ยืดสเกลได้เอง ราคาที่อยู่นอกกรอบ
     * จะถูกแปลงเป็นตำแหน่งที่เลยแผงราคาลงไป แล้ววาดทับแผง Volume/RSI/MACD
     * และทะลุออกนอกกราฟไปเลย (ผู้ใช้เจอตอนยืด 4.1× บนไอแพด)
     *
     * กันด้วย clip ที่เดียว ดีกว่าไปใส่เงื่อนไข "ถ้าเกินกรอบให้ข้าม"
     * ในทุกจุดที่วาด ซึ่งจะลืมจุดใดจุดหนึ่งเสมอเมื่อเพิ่มของใหม่
     */
    g.save();
    g.beginPath();
    g.rect(padL, padT, W - padR - padL, priceH);
    g.clip();
    ...
    /* เลิกตัดขอบตรงนี้ — ป้ายของแผน/ไม้ ต้องวาดล้ำไปบนแถบราคาด้านขวาได้
       ส่วนเส้นของมันมีการเช็คกรอบราคาอยู่แล้วในโค้ดข้างล่าง */
    g.restore();
    ...
    if (this.position && this.position.entry > 0) {
      ...
      /*
       * เส้นที่หลุดออกนอกจอ ต้องบอกว่าอยู่ทางไหนและไกลแค่ไหน
       *
       * ช่องโหว่ที่เกิดขึ้นตอนใส่การยืดแกนราคา: พอยืดแรง ๆ เส้น SL กับเป้าหลุดจอ
       * เหลือแต่เส้นราคาเข้า คนถือไม้อยู่จึงมองไม่เห็นว่าอีกกี่ดอลลาร์ถึงจะโดน SL
       * ซึ่งเป็นตัวเลขที่สำคัญที่สุดตอนมีเงินอยู่ในตลาด
       */
      for (const r of rows) {
        if (r.p >= min && r.p <= max) continue;
        const above = r.p > max;
        /* ป้ายบนต้องหลบป้ายบอกสเกล ซึ่งอยู่มุมบนซ้ายและวาดทีหลัง
           ไม่งั้นสองอันทับกันจนอ่านไม่ออกทั้งคู่ (เจอตอนทดสอบ: "+ 90.00" เหลือ "0.00") */
        const zoomed = Math.abs((this.view.priceZoom || 1) - 1) > 0.02;
        const y = above ? padT + (zoomed ? 32 : 10) : padT + priceH - 8;
        const gap = Math.abs(r.p - px);
        const txt = `${above ? '▲' : '▼'} ${r.txt}  (ห่าง ${gap.toFixed(2)})`;
        g.font = '10px ui-monospace, monospace';
        const tw = g.measureText(txt).width;
        const bx = W - padR - 8 - tw - 10;
        g.fillStyle = 'rgba(11,16,32,.9)';
        g.strokeStyle = r.col; g.lineWidth = 1;
        g.setLineDash([3, 2]);
        roundRect(g, bx, y - 8, tw + 10, 16, 4); g.fill(); g.stroke();
        g.setLineDash([]);
        g.fillStyle = r.col; g.textAlign = 'left';
        g.fillText(txt, bx + 5, y + 3.5);
      }
