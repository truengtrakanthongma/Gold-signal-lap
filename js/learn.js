...
/**
 * ควรเพิ่มขนาดไม้เมื่อสัญญาณ "ชัด" ไหม — และเพิ่มได้เท่าไร
 *
 * การเพิ่มไม้ตอนมั่นใจเป็นวิธีทำเงินที่ถูกต้อง *ถ้า* ความมั่นใจนั้นวัดได้จริง
 * ถ้าไม่จริง มันคือการเร่งขาดทุนให้เร็วขึ้นเป็นเท่าตัว ซึ่งแย่กว่าไม่เพิ่มเลย
 * คนส่วนใหญ่พังตรงนี้เพราะเชื่อความรู้สึกว่า "ไม้นี้ชัวร์"
 *
 * ที่นี่จึงไม่เชื่อคะแนนโดยอัตโนมัติ แต่ไปดูจากผลจริงในอดีตว่า
 * ไม้คะแนนสูงเคยทำเงินได้ดีกว่าไม้ทั่วไปหรือเปล่า และดีกว่าแบบที่ไม่ใช่ความบังเอิญ
 * (ใช้การสุ่มซ้ำแบบเดียวกับที่ใช้ตัดสินน้ำหนักปัจจัย)
 *
 * ถ้าหลักฐานไม่ผ่าน จะคืนตัวคูณ 1 พร้อมบอกเหตุผล ไม่ใช่เงียบ ๆ แล้วไม่ทำอะไร
 * ผู้ใช้จะได้รู้ว่า "ยังไม่เพิ่มให้ เพราะอะไร" ไม่ใช่นึกว่าฟีเจอร์พัง
 */
export function confidenceScale(bt, score, opts = {}) {
  const threshold = opts.threshold;
  /* 1.3 ไม่ใช่เลขสวย ๆ แต่มาจากการวัด: ที่ 1.5 เท่า มีไม้เข้าเงื่อนไขแค่ 108 จาก 2,772
     ซึ่งน้อยเกินกว่าจะสรุปอะไรได้ ส่วน 1.3 ได้ 542 ไม้ พอให้ด่านตัดสินอย่างมีความหมาย */
  const boostAt = opts.boostAt === undefined ? 1.3 : opts.boostAt;   // กี่เท่าของเกณฑ์ถึงเรียกว่าชัด
  const maxMult = opts.maxMult === undefined ? 2 : opts.maxMult;     // เพิ่มไม้ได้มากสุดกี่เท่า
  const minN = opts.minN === undefined ? 20 : opts.minN;             // ต้องมีตัวอย่างอย่างน้อยเท่าไร
  const conf = opts.conf === undefined ? 0.9 : opts.conf;            // ต้องมั่นใจเท่าไรว่าดีกว่าจริง

  const no = (why) => ({ mult: 1, boosted: false, why });

  if (!Number.isFinite(threshold) || threshold <= 0) return no('ไม่รู้เกณฑ์สัญญาณ จึงไม่เพิ่มขนาดไม้');
  if (!bt || !bt.trades || !bt.trades.length) {
    return no('ยังไม่มีผลทดสอบย้อนหลัง — กดปุ่ม "ทดสอบย้อนหลัง" ก่อน แล้วระบบจะรู้ว่าคะแนนสูงคุ้มให้เพิ่มไม้ไหม');
  }

  const a = Math.abs(score);
  const line = threshold * boostAt;
  if (a < line) return no(`คะแนน ${a.toFixed(1)} ยังไม่ถึงระดับที่นับว่าชัด (ต้อง ${line.toFixed(0)} ขึ้นไป)`);

  const strong = [];
  const rest = [];
  for (const t of bt.trades) (t.absScore >= line ? strong : rest).push(t.rMultiple);
  if (strong.length < minN) return no(`ไม้คะแนน ≥ ${line.toFixed(0)} ในอดีตมีแค่ ${strong.length} ไม้ (ต้องการ ${minN}) ยังสรุปไม่ได้`);
  if (rest.length < minN) return no(`ไม้คะแนนปกติมีแค่ ${rest.length} ไม้ ยังไม่มีอะไรให้เทียบ`);

  const mean = (xs) => xs.reduce((sum, x) => sum + x, 0) / xs.length;
  const mStrong = mean(strong);
  const mRest = mean(rest);
  const p = probBetter(rest, strong);

  if (!(mStrong > mRest) || p === null || p < conf) {
    return no(`คะแนนสูงยังไม่พิสูจน์ว่าให้ผลดีกว่า — ${mStrong.toFixed(2)}R เทียบ ${mRest.toFixed(2)}R `
      + `มั่นใจ ${p === null ? '—' : (p * 100).toFixed(0) + '%'} (ต้องการ ${(conf * 100).toFixed(0)}%)`);
  }

  /* ยิ่งเกินเส้นมาก ยิ่งเพิ่มมาก แต่ชนเพดานที่ maxMult เสมอ
     ไต่ขึ้นเต็มเพดานเมื่อเกินเส้นไปอีกครึ่งหนึ่งของเกณฑ์ */
  const ramp = Math.min(1, (a - line) / (threshold * 0.5));
  return {
    mult: 1 + (maxMult - 1) * ramp,
    boosted: true, p, n: strong.length, strongR: mStrong, restR: mRest, line,
    why: `ไม้คะแนน ≥ ${line.toFixed(0)} ในอดีต ${strong.length} ไม้ ทำได้เฉลี่ย ${mStrong.toFixed(2)}R ต่อไม้ `
      + `เทียบกับ ${mRest.toFixed(2)}R ของไม้ทั่วไป — มั่นใจ ${(p * 100).toFixed(0)}% ว่าไม่ใช่ความบังเอิญ`,
  };
}
