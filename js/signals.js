...
/*
 * ทำไม "ท่าจากตำรา" (classic.js) ไม่มีน้ำหนักในนี้
 *
 * เคยใส่ไว้ที่ 10 แล้วผู้ใช้ทักว่าระบบเริ่มไม่แม่น ตามไปวัดแล้วเจอสาเหตุ:
 * คะแนนคิดจาก (คะแนนดิบ ÷ น้ำหนักรวม) × 100 การเพิ่มปัจจัยเข้าไปจึงเพิ่ม
 * "ตัวหาร" ทันที แต่ตัวจับรูปแบบพวกนี้ทำงานแค่ 1 ครั้งใน 20-134 แท่ง
 * ตัวตั้งจึงแทบไม่เคยเพิ่มตาม ผลคือคะแนน "ทุกแท่ง" ต่ำลง 7.7%
 * ทั้งที่สภาพตลาดไม่ได้เปลี่ยน — สเกลที่ผู้ใช้คุ้นมือมาทั้งหมดเลื่อนไปเงียบ ๆ
 *
 * และมันไม่เคยพิสูจน์ตัวเองด้วย: วัดบนข้อมูลควบคุมแล้วได้ผลอยู่ในช่วงความคลาดเคลื่อน
 * ปัจจัยที่ยังพิสูจน์ไม่ได้ ไม่ควรได้สิทธิ์ไปลดคะแนนของปัจจัยที่พิสูจน์แล้ว
 *
 * ตัวจับรูปแบบยังอยู่ครบใน classic.js และยังโชว์เป็นข้อมูลประกอบได้
 * ถ้าวันหนึ่งวัดบนข้อมูลจริงของผู้ใช้แล้วพบว่าช่วยจริง ค่อยใส่น้ำหนักกลับมา
 */
...
export function scoreAt(ctx, i) {
  ...
  /*
   * เพิ่มปัจจัยหนึ่งตัวเข้าไปในคะแนน
   *
   * ถ้าคีย์นั้นไม่มีน้ำหนักกำหนดไว้ ต้องบันทึกเป็น "ข้อมูลประกอบ" ไม่ใช่คิดคะแนน
   * ของเดิมใช้ W[key] ตรง ๆ พอคีย์ไม่มีจะได้ undefined แล้วคูณออกมาเป็น NaN
   * ซึ่งไหลไปทำให้คะแนนรวมทั้งแท่งเป็น NaN โดยไม่มีอะไรฟ้อง — เจอตอนถอด classic ออก
   */
  const push = (key, name, side, strength, reason, extra = {}) => {
    if (!side || strength <= 0) return;
    const w = W[key];
    const known = Number.isFinite(w);
    factors.push({
      key, name, side, strength: clamp(strength, 0, 1), weight: known ? w : 0,
      contribution: known ? w * clamp(strength, 0, 1) * side : 0,
      reason, ...(known ? {} : { info: true }), ...extra,
    });
  };
   ...
   *
   * ตอนนี้ทั้งกลุ่มเป็น "ข้อมูลประกอบ" ล้วน ไม่มีน้ำหนักในคะแนน (ดูเหตุผลที่ WEIGHTS)
   * ยังบอกผู้ใช้ว่าเจออะไร แต่ไม่ไปลดคะแนนของปัจจัยที่พิสูจน์ตัวเองแล้ว
   */
  const classics = [
...
export function buildSetup(ctx, i, scored, opts = {}) {
  ...
  /*
   * สิ่งที่ต้องทำ "หลังเข้าไม้แล้ว" — เดิมแผนเงียบเรื่องนี้ทั้งหมด
   *
   * ผู้ใช้จริงเสียไม้ไปเพราะตรงนี้: เข้าขายถูกทาง SL เดิมไม่เคยถูกแตะเลย
   * แต่ขยับ SL มาไว้ห่างจากราคาเข้าแค่ 0.50 (0.04 เท่าของระยะเสี่ยง)
   * ราคาย่อกลับตามปกติก็ปิดไม้ทิ้งทันที ทั้งที่แผนเดิมยังไม่ผิดสักนิด
   *
   * วัดจากไม้ย้อนหลัง 2,772 ไม้: การขยับ SL มาที่ทุนหลังผ่าน 1R (ซึ่งมีวินัย
   * กว่าที่เขาทำมาก) ยังทำให้กำไรเฉลี่ยลดจาก +0.033R เหลือ -0.003R ต่อไม้
   * และเปลี่ยนไม้ที่กำลังไปได้ดี 901 ไม้ ให้กลายเป็นเสมอทุน
   *
   * แผนจึงต้องพูดเรื่องนี้ออกมาตรง ๆ ไม่ใช่ปล่อยให้เดาเอง
   */
  const manage = [
    'ตั้ง SL กับ TP ให้ครบตั้งแต่ตอนส่งคำสั่ง แล้วปล่อยให้โบรกเกอร์ปิดให้เอง',
    'อย่าขยับ SL เข้าใกล้ราคาเข้าระหว่างที่ไม้ยังวิ่ง — วัดจากไม้ย้อนหลังแล้ว '
      + 'การย้ายมาที่ทุนทำให้กำไรเฉลี่ยต่อไม้ลดลง เพราะราคาย่อปกติก็เตะออกก่อนถึงเป้า',
    `ถ้าจะขยับจริง ๆ ให้รอจนราคาไปถึง ${tp1.toFixed(2)} ก่อน (กำไรเท่ากับระยะที่เสี่ยงพอดี) `
      + 'ไม่ใช่ขยับตั้งแต่ราคาเพิ่งขยับไปเล็กน้อย',
    `ไม้นี้ผิดทางจริงเมื่อราคา${side > 0 ? 'หลุดลงต่ำกว่า' : 'ทะลุขึ้นเหนือ'} ${sl.toFixed(2)} เท่านั้น `
      + 'ก่อนถึงจุดนั้นคือความแกว่งปกติ ไม่ใช่สัญญาณให้รีบหนี',
  ];

  return {
    side, entry: entryPrice, sl, tp1, tp2, tp3, tpMain, mainR, manage,
