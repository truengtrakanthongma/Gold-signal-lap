...
export function runBacktest(ctx, opts = {}) {
  ...
  while (i < stopAt) {
    ...
    for (let j = entryIdx; j <= Math.min(entryIdx + o.maxHold, n - 1); j++) {
      ...
      /*
       * *** จุดตัดขาดทุนต้องมีผลเสมอ เว้นแต่ท่านั้นย้ายจุดตัดไปแล้วจริง ๆ ***
       *
       * บั๊กที่เคยมี: เงื่อนไขเดิมคือ (hitSL && !hit1R) ซึ่งแปลว่าพอราคาแตะ 1R
       * ปุ๊บ จุดตัดขาดทุนก็เลิกทำงานทันทีทุกท่า แต่ท่า 'full' ไม่ได้ย้ายจุดตัดไปไหนเลย
       * ไม้เลยวิ่งทะลุ SL ลงไปเรื่อย ๆ จนหมดเวลาถือ วัดแล้วเจอไม้ที่ขาดทุน -124R
       * คือ 124 เท่าของเงินที่ตั้งใจเสี่ยง — ไม้เดียวล้างพอร์ตได้สบาย
       *
       * ท่าที่ย้ายจุดตัดจริงมีแค่ partial กับ full-be (ย้ายมาที่ทุนหลังผ่าน 1R)
       * สองท่านั้นมีการเช็คจุดตัดใหม่ของตัวเองอยู่ข้างล่างแล้ว
       */
      const stopMoved = hit1R && (style === 'partial' || style === 'full-be');
...
function summarize(trades, o, missed = []) {
  ...
  /*
   * "ชนะ" ต้องแปลว่าได้เงิน ไม่ใช่แค่ราคาเคยวิ่งไปแตะเป้า
   *
   * ของเดิมนับไม้ที่ hit1R คือ "เคยแตะ 1R" ซึ่งไม่เท่ากับปิดไม้แล้วได้กำไร
   * ไม้ที่แตะ 1R แล้วย้อนกลับมาโดน SL ถูกนับเป็นชนะทั้งที่เสียเงินจริง
   * วัดแล้วท่า full-be รายงาน 65.3% ทั้งที่ได้เงินจริงแค่ 44.5% — ต่างกัน 20 จุด
   * ตัวเลขที่โฆษณาตัวเองเกินจริงแบบนี้ อันตรายกว่าตัวเลขที่ต่ำแต่จริง
   */
  const wins1R = trades.filter((t) => t.rMultiple > 0).length;
  const reached1R = trades.filter((t) => t.hit1R).length;
  ...
  const factors = factorKeys.map((key) => {
    ...
    const wr = (list) => (list.length ? (list.filter((t) => t.rMultiple > 0).length / list.length) * 100 : null);
  ...
  return {
    ...
    stats: {
      ...
      // "เคยวิ่งไปถึงเป้าไหม" ยังมีประโยชน์ แต่ต้องแยกช่องกับ "ได้เงินไหม" ให้ชัด
      reach1RRate: n ? (reached1R / n) * 100 : null,
      /*
...
export function evaluateTarget(trades, targetR, costR = 0) {
  ...
  /* ขาดทุนสะสมลึกสุดของลำดับไม้ชุดนี้ — ต้องเดินตามลำดับเวลาจริง
     ใช้เป็นตัวหารของ "กำไรเทียบความเจ็บ" ซึ่งเป็นตัวตัดสินว่าจะเลือกค่าไหน */
  let run = 0, peak = 0, maxDD = 0;
  for (const t of trades) {
    const r = (t.favBeforeStop || 0) >= targetR ? targetR - costR
      : (t.result === 'loss' ? -1 - costR : Math.max(-1, Math.min(targetR, t.rMultiple)) - costR);
    run += r;
  ...
  return {
    ...
    expectancy: total / trades.length, totalR: total, maxDD,
    /*
     * ตัวตัดสินหลัก: ได้กำไรเท่าไร ต่อความเจ็บหนึ่งหน่วย
     *
     * ทำไมไม่ใช้กำไรรวมเฉย ๆ: วัดแล้วกำไรรวมสูงสุดมาจากการเก็บไม้เยอะ ๆ
     * ซึ่งได้อัตราชนะ 30% และขาดทุนสะสมลึก 102R — คนจริง ๆ ทนไม่ไหวและเลิกก่อน
     * กลยุทธ์ที่คนไม่ทำตาม มีค่าเท่ากับศูนย์ ไม่ว่าตัวเลขบนกระดาษจะสวยแค่ไหน
     *
     * ทำไมไม่ใช้กำไรต่อไม้เฉย ๆ: มันเชียร์ให้เข้าน้อยที่สุดเท่าที่จะน้อยได้
     * จนเหลือไม้ไม่กี่ไม้ ซึ่งสรุปอะไรไม่ได้เลยทางสถิติ
     *
     * กำไรหารความเจ็บ ตอบทั้งสองข้อพร้อมกัน วัดแล้วได้ชนะ 42% ความเจ็บ 15R
     * เทียบกับ 30% / 102R ของการไล่กำไรรวมอย่างเดียว
     */
    score: total / Math.max(maxDD, 1),
  };
}
...
export function optimizeExits(ctx, opts = {}) {
  ...
  /* ขยายช่วงเกณฑ์ขึ้นไปถึง 60 — วัดแล้วพบว่าจุดที่ดีที่สุดมักอยู่แถว 45-55
     ของเดิมตัดที่ 50 ทำให้มองไม่เห็นว่าเข้มกว่านั้นดีกว่าไหม */
  const thresholds = o.thresholds || [20, 25, 30, 35, 40, 45, 50, 55, 60];
  ...
  for (const g of grid) {
    ...
    /*
     * ใช้ "กำไรรวม" เป็นตัวตัดสิน ไม่ใช่ "กำไรต่อไม้"
     *
     * เดิมขั้นนี้ตัดสินด้วยกำไรต่อไม้ แต่ขั้นเลือกวิธีเข้า-ออกตัดสินด้วยกำไรรวม
     * สองเป้าหมายในตัวหาค่าเดียวกันจึงดึงกันเอง: ขั้นแรกชอบเกณฑ์สูง ๆ ที่ได้ไม้น้อย
     * แต่คุณภาพสูง ขั้นหลังชอบท่าที่เก็บได้เยอะ ผลลัพธ์เลยไม่ใช่จุดที่ดีที่สุดของทั้งคู่
     *
     * กำไรรวมคือเงินที่เข้าบัญชีจริงในช่วงเวลาเท่ากัน จึงเป็นตัวที่ตรงกับสิ่งที่ผู้ใช้ได้
     */
    g.robust = neighbours.reduce((a, x) => a + x.score, 0) / neighbours.length;
