...
export function rsi(values, period = 14) {
  ...
  /*
   * ไม่มีขาดทุนเลย ไม่ได้แปลว่า "ซื้อมากเกินสุดขีด" เสมอไป
   *
   * ของเดิมเขียน avgL === 0 ? 100 ซึ่งถูกเฉพาะตอนราคาขึ้นล้วน
   * แต่ตอนราคา "ไม่ขยับเลย" ก็ไม่มีขาดทุนเหมือนกัน แล้วได้ 100 ไปด้วย
   * ทั้งที่ตลาดนิ่งสนิทคือกลาง ๆ ไม่ใช่ร้อนแรงสุดขีด
   *
   * ทำไมสำคัญ: RSI 100 ทำให้ระบบดัน "แรงขาย" ออกมาจากตลาดที่ไม่ได้ขยับเลย
   * และกรณีที่เจอบ่อยที่สุดคือ "ราคาค้าง" — แหล่งข้อมูลส่งราคาเดิมซ้ำ ๆ
   * ซึ่งเป็นตอนที่ระบบควรเงียบที่สุด กลับกลายเป็นตอนที่มันตะโกนดังที่สุด
   */
  const val = (g, l) => {
    if (l === 0) return g === 0 ? 50 : 100;   // ไม่ขยับเลย = กลาง · ขึ้นล้วน = 100
    return 100 - 100 / (1 + g / l);
  };
  out[period] = val(avgG, avgL);
